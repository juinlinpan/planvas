...
- Markdown note editing should autosave about every 5 seconds, provide a manual `Save` button for immediate persistence, flush immediately when the browser window/tab is left, and flush when leaving the markdown editor view. Returning to a Page or focusing the window should only check whether project notes changed on disk; it must not apply a project-note refresh automatically, clear the page board cache, reload the page list, or remount the active Page.
...
- If the background project-note check detects a difference, the Notes refresh button should show an update indicator. Clicking that refresh button applies the updated project notes once and clears the indicator.
...
- Board-state autosave must not refresh project notes or replace the Canvas item array after every save. It may refresh project notes only when the backend actually creates or changes markdown-backed `note_paper` metadata such as the generated `noteFile`.
