...
- Markdown note editing should autosave about every 5 seconds, provide a manual `Save` button for immediate persistence, flush immediately when the browser window/tab is left, and flush when leaving the markdown editor view. Returning to a Page or focusing the window checks whether project notes changed on disk. If a note changed on disk and has no active unsaved draft in the app, the changes are automatically applied to the in-memory note state (updating the canvas placements and open tabs) silently to prevent stale state overwrites. It must not apply a project-note refresh automatically if there is an active draft. Page XML files (both semantic and presentation) do not store volatile `updated_at` attribute in their root elements to prevent git diff pollution.
- If a markdown editor opens with an unsaved in-memory draft, it must schedule autosave immediately instead of waiting for another keystroke. Failed autosave attempts should leave the draft in memory and retry on the normal autosave interval.
- If a background project-note check detects a difference for a note that has an active unsaved draft in the app, the Notes refresh button shows an update indicator. Clicking that refresh button applies the updated project notes once, resolving the conflict, and clears the indicator.
- A markdown note tab that remains open must preserve its latest in-memory draft while the user switches to another workspace tab, even if the autosave interval has not fired yet. Successful note saves must update the frontend project-note state immediately from the save API response and must not trigger a project-note refresh cycle.
- Board-state autosave must not refresh project notes or replace the Canvas item array after every save. It may refresh project notes only when the backend actually creates or changes markdown-backed `note_paper` metadata such as the generated `noteFile`, or when the user edits markdown content on the Page.
- Board-state autosave must send `note_paper.content` as `null` for existing markdown-backed notes whose content did not change in the Page editor, so geometry/page saves cannot overwrite external `.md` edits.
